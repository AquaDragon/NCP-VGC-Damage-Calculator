...
// Modified to allow calcfarm to save targets as a different set
function processSave(string, spreadName, sidebarUsed, setGen = gen, calcfarm = false) {
    let sets = []; 
    if (calcfarm) {
        sets = [];
    }
    ...
    try {
        for (var a = 0; a < numPokemon.length; a++) {
            var spreadNameID = spreadName;
            if (calcfarm) {
                spreadNameID = spreadName + " #" + a;
            }
        ...
        10	- Move Name
          ...
          */
            ...
            saveSets(setGen, customFormat, species, spreadNameID);
            if (sidebarUsed && a < 6) {
            ...
            if (calcfarm) {
                sets.push([species, spreadNameID]);
            }
        }

        if (setGen == gen) loadSetdexScript();
...
function GenerateCsv() {
    ...
    for (var j = 0; j < yourTeamSets.length; ++j) {
        ...
        offensiveResults[p1.shortName] = [];
        defensiveResults[p1.shortName] = [];
        leftPokemon.push(p1);
    }    
